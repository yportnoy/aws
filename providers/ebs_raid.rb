...
def mount_device(raid_dev, mount_point, mount_point_owner, mount_point_group, mount_point_mode, filesystem, filesystem_options)
  ...
  ruby_block "find md device in #{new_resource.name}" do
    block do
      ...
      Dir.glob("/dev/md[0-9]*").each do |dir|
        Chef::Log.error("More than one /dev/mdX found.") unless md_device.nil?
        unless ::File.lstat(dir).rdev == ::File.lstat(mount_point).dev
          md_device = dir
          break
        end
      end
      ...
      Chef::Log.info("Adding #{md_device} to fstab")
      
      unless system("grep #{mount_point} /etc/fstab | grep #{md_device}")
        system("echo '#{md_device} #{mount_point} #{filesystem} noatime,nobootwait 0 2' >> /etc/fstab")
      end
    end
  end
end
...
# Diskset      I.E sdi (which creates sdi-sdz<n>
